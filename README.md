...
The script is mostly controlled by a set of environment variables, all of
...
[Now you're ready to do this preparation work](README2nd/Preconfigure.md).
...
## Networking and DNS for the nodes

Each node gets its IP address via DHCP to the corresponding dnsmasq of its
virtual intranet.  **$FAME_OCTETS123** defines the first three octets of
the RFC1914 address; the default is "192.168.42" .
...
There are several ways to resolve node hostnames from the QEMU host:
...
You can ssh to the node as the normal **$FAME_USER**.  If you set your $HOME/.ssh/config file correctly using the id_rsa.nophrase private key the ssh occurs without further confirmation.
